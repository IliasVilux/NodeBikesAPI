...
<template>
  <div class="w-full bg-text-primary">
    <div class="lg:hidden text-white flex items-center justify-center gap-1 cursor-pointer py-4" @click="isFiltersOpenMobile = !isFiltersOpenMobile">
      <h3 class="text-2xl">Filtros</h3>
      <Svg name="arrow-simple-down" class="size-7 transition-transform" :class="{ 'rotate-180': isFiltersOpenMobile }" />
    </div>
    <div class="max-w-screen-2xl mx-auto grid-cols-1 lg:grid-cols-3 gap-x-4" :class="{ 'hidden': !isFiltersOpenMobile, 'lg:grid': true }">
      <div
      ...
      >
        ...
        <div
          ...
          :class="{ 'bg-white rounded-t-2xl text-primary shadow-xl': openDropdowns[filter.id], 'text-white': !openDropdowns[filter.id] }"
          @click="toggleDropdown(filter.id)"
        >
          <p class="uppercase">{{ filter.name }}</p>
          <Svg name="arrow-simple-down" class="size-7 transition-transform" :class="{ 'rotate-180': openDropdowns[filter.id] }" />
        </div>
        ...
        >
          <p v-for="(option, i) in filter.options" :key="i" class="px-4 py-4 lg:py-2 hover:font-medium uppercase">
            {{ option.name }}
          </p>
        </div>
      </div>
